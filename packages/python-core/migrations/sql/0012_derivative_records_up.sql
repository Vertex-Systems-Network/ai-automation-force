CREATE TABLE core.derivative_records (
    ...
    CONSTRAINT ck_derivative_completion CHECK (
        ...
        OR
        (status <> 'completed' AND output_asset_id IS NULL AND output_storage_object_id IS NULL)
    ),
    CONSTRAINT ck_derivative_chronology CHECK (
        completed_at IS NULL OR completed_at >= created_at
    ),
    CONSTRAINT uq_derivative_operation UNIQUE (project_id, source_asset_id, operation_fingerprint)
);

CREATE INDEX idx_derivative_source_status
    ON core.derivative_records (source_asset_id, status, created_at, id);
